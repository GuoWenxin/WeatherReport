...
public class ChooseAreaActivity extends Activity {
	...
	private void queryProvinces()
	{
		...
		if (provinceList.size()>0) {
			...
			titTextView.setText(R.string.china);
	...
	{
		Log.d("aaaaa", Integer.toString(selectedProvince.getId()));
		cityList=coolWeatherDB.loadCitys(selectedProvince.getId());
		Log.d("aaaaa", Integer.toString(cityList.size()));
		if (cityList.size()>0) {
			dataList.clear();
	...
	{
		countyList=coolWeatherDB.loadConty(selectedCity.getId());
		...
		HttpUtil.sendHttpRequest(address, new HttpCallbackListener() {
			...
			public void onError(Exception e) {
				...
				runOnUiThread(new Runnable() {
					...
					public void run() {
						closeProgressDialog();
						Toast.makeText(ChooseAreaActivity.this, R.string.loadfail, Toast.LENGTH_SHORT).show();
					}
				});
			}
		});
